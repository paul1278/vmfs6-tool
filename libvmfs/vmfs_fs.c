...
static int vmfs_fsinfo_read(vmfs_fs_t *fs)
{
   ...
   printf("FS vol_version: %u\n", fsi->vol_version);
   printf("FS version: %u\n", fsi->version);
   printf("FS block_size: %lu\n", fsi->block_size);
   printf("FS subblock_size: %u\n", fsi->subblock_size);
   printf("FS fdc_header_size: %u\n", fsi->fdc_header_size);
   printf("FS fdc_bitmap_count: %u\n", fsi->fdc_bitmap_count);
   printf("FS label: %s\n", fsi->label);
   return (0);
}
                                          ...
                                          char *desc)
{
   ...
   if ((bitmap->bmh.total_items + bitmap->bmh.items_per_bitmap_entry - 1) /
           bitmap->bmh.items_per_bitmap_entry >
       max_entry)
   {
