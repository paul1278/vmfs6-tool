...
static int vmfs_volinfo_read(vmfs_volume_t *volume)
{
   ...
   printf("READING LVM SIZE FROM: %d\n", VMFS_LVMINFO_OFS_SIZE);
...
vmfs_volume_t *vmfs_vol_open(const char *filename, vmfs_flags_t flags)
{
   ...
   if (vmfs_volinfo_read(vol) == -1)
      goto err_open;
   vol->flags.debug_level = 1000;
   printf("VMFS versionssss: %d\n", vol->vol_info.version);
   printf("Blocks: %d\n", vol->vol_info.blocks);
   printf("LVM_SIZE: %ld\n", vol->vol_info.lvm_size);
   printf("SIZE: %ld\n", vol->vol_info.size);
   char uuid_str[M_UUID_BUFLEN];
   printf("Volume UUID: %s\n", m_uuid_to_str(vol->vol_info.uuid, uuid_str));
   printf("LUN ID: %d\n", vol->vol_info.lun);
   char uuid_str2[M_UUID_BUFLEN];
   printf("LVM UUID: %s\n", m_uuid_to_str(vol->vol_info.lvm_uuid, uuid_str2));
   printf("Number of segments: %u\n", vol->vol_info.num_segments);
   printf("Number of extents: %u\n", vol->vol_info.num_extents);
